---
title: "Gestures — Touch Input Configuration API"
...
```kotlin
fun Gestures.none(): SceneGestures
```
...
`SceneGestures` is a `@ConsistentCopyVisibility @Immutable` data class. It is the value type consumed by `Scene`. You obtain instances exclusively through the `Gestures` factory; direct construction is `internal`.
