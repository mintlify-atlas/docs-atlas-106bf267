---
title: "CameraState — Orbit Camera State API"
...
<ParamField path="zoom" type="Float" default="1f">
  Initial visual magnification factor. `1.0` is the default field of view. Values above `1.0` zoom in (objects appear larger); values below `1.0` zoom out. Clamped to `[MIN_ZOOM, MAX_ZOOM]` which are `0.3` and `4.0` respectively.
</ParamField>
...
Snaps the camera to an absolute zoom level. The value is clamped to `[MIN_ZOOM, MAX_ZOOM]` (0.3–4.0) before being applied.
